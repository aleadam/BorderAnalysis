  ...
  implements PlugInFilter
{
  ...
  public void save(ProfilesData pd)
  {
    try
    {
      ...
      writer.newLine();
      writer.write("Background (Average intensity below 70% percentile): ," + 
        pd.getAverageBelow(6));
      writer.newLine();
      ...
      writer.newLine();
      writer.write("Peak range: ," + pd.getMinPeak() + " - " + 
        pd.getMaxPeak());
      ...
      for (int i = 0; i < pd.peakVal.length; i++)
      {
        ...
        for (int j = 0; j < pd.peakPos[i].length; j++) {
          if (i < pd.peakVal.length / 2) {
            writer.write("(" + pd.lines[i].x1 + ":" + 
              pd.peakPos[i][j] + "),");
          } else {
            writer.write("(" + pd.peakPos[i][j] + ":" + 
              pd.lines[i].y1 + "),");
          }
        }
        writer.newLine();
